 ...
 * Rev.:    		3.01
 * Author:  		LIFEI
 * LastUptime:	2016-11-30 11:00:08
 *-----------------------------------------------------------------------------*/
/*Includes***************************************/
#ifndef _BUTTONS_C
#define _BUTTONS_C
...
/*Private define ***************************************/
#define BUTTON_COUNT (sizeof(Pin_BUTTON)/sizeof(GPIO_PIN_ID))
...
osThreadId tid_Thread_Button;      
osThreadDef(Thread_Button, osPriorityNormal, 1, 0);

int Init_Thread_Button (void) {
	
  tid_Thread_Button = osThreadCreate (osThread(Thread_Button), NULL);
  if (!tid_Thread_Button) return(-1);
  return(0);
}
...
{	
	...
	while(1){
		if(bState==HIGH2LOW){
		Button_MASK_main = GetState_Pin_BUTTON(Button_MASK_main,HIGH2LOW,count);
		}
		else if(bState==LOW2HIGH){
		Button_MASK_main = GetState_Pin_BUTTON(Button_MASK_main,LOW2HIGH,count);
		}
		osDelay(100);
		if(Button_MASK_main){
			break;
