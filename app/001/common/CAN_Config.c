...
#define CAN_STM32_A 1
#define CAN_STM32_B 0
#define CAN_STM32_C 0
#define CAN_STM32_D 0
#define CAN_STM32_E 0

#define CAN_STM32_A_EXTENDED_ID 0x11111111U
#define CAN_STM32_B_EXTENDED_ID 0x12222222U
#define CAN_STM32_C_EXTENDED_ID 0x13333333U
#define CAN_STM32_D_EXTENDED_ID 0x14444444U
#define CAN_STM32_E_EXTENDED_ID 0x15555555U
...
bool CAN_Initialize (void) {
  ...
  if ((rx_obj_idx == 0xFFFFFFFFU) || (tx_obj_idx == 0xFFFFFFFFU)) { return false; }
	...
	#if (CAN_STM32_A!= 0)
	status = ptrCAN->ObjectSetFilter(rx_obj_idx, ARM_CAN_FILTER_ID_EXACT_ADD, ARM_CAN_EXTENDED_ID(CAN_STM32_A_EXTENDED_ID), 0U);
  if (status != ARM_DRIVER_OK) { return false; }
	#elif (CAN_STM32_B!=0)
	status = ptrCAN->ObjectSetFilter(rx_obj_idx, ARM_CAN_FILTER_ID_EXACT_ADD, ARM_CAN_EXTENDED_ID(CAN_STM32_B_EXTENDED_ID), 0U);
  if (status != ARM_DRIVER_OK) { return false; }
	#elif (CAN_STM32_C!=0)
	status = ptrCAN->ObjectSetFilter(rx_obj_idx, ARM_CAN_FILTER_ID_EXACT_ADD, ARM_CAN_EXTENDED_ID(CAN_STM32_C_EXTENDED_ID), 0U);
  if (status != ARM_DRIVER_OK) { return false; }
	#elif (CAN_STM32_D!=0)
	status = ptrCAN->ObjectSetFilter(rx_obj_idx, ARM_CAN_FILTER_ID_EXACT_ADD, ARM_CAN_EXTENDED_ID(CAN_STM32_D_EXTENDED_ID), 0U);
  if (status != ARM_DRIVER_OK) { return false; }
	#elif (CAN_STM32_E!=0)
	status = ptrCAN->ObjectSetFilter(rx_obj_idx, ARM_CAN_FILTER_ID_EXACT_ADD, ARM_CAN_EXTENDED_ID(CAN_STM32_E_EXTENDED_ID), 0U);
  if (status != ARM_DRIVER_OK) { return false; }
	#endif
...
void CAN_Thread (void const *arg) {
  ...
  memset(&tx_msg_info, 0U, sizeof(ARM_CAN_MSG_INFO));   // Clear message info structure
	tx_msg_info.id = ARM_CAN_EXTENDED_ID(CAN_STM32_B_EXTENDED_ID);
