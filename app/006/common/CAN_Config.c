#ifndef _CAN_CONFIG_C
#define _CAN_CONFIG_C
#include "can_config.h"
#define CAN_STM32_rx_EXTENDED_ID 0x12222222U
  ...
  if (event == ARM_CAN_EVENT_RECEIVE) {         // If receive event
    if (obj_idx == rx_obj_idx) {                // If receive object event
      ptrCAN->MessageRead(rx_obj_idx, &rx_msg_info, rx_data, 8U);
			...
			if(memcmp(rx_data,cantype.crx_instruct[0],8)==0){
				cantype.fcrx_instruct=1;
			}
			if(memcmp(rx_data,cantype.crx_state[0],8)==0){
				cantype.fcrx_state=1;
			}
    }
  }
...
void CAN_Thread (void const *arg) {
  ...
  while (1) {
		...
		if(cantype.fctx_state==1){
		memcpy(tx_data,cantype.ctx_instruct[0],8U);
		ptrCAN->MessageSend(tx_obj_idx, &tx_msg_info, tx_data, 8U);
		osDelay(1000U);
		}
		
		if(cantype.fcrx_instruct==1){
			//func_ 
			cantype.fcrx_instruct=0;
		}
		if(cantype.fcrx_state==1){
			//func_
			cantype.fcrx_state=0;
		}
  }
}	
